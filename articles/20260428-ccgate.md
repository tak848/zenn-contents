...
- 自分の環境では1ヶ月約2000件のPermission確認のうち、基本的に97〜100%近くが自動化された
:::
...
つらかったのは、たんにYesを押す回数だけではありません。Plan modeなどで動くExplore Agentが、コードベースを調べるために`$()`、`&&`、`||`、pipeなどを使ったシェル芸で検索してくることがあります。中身はAllow Listに入れているコマンドの組み合わせでも、Claude Code側の解析器がうまく扱えず、Permission許可を求めてくることがあります。

こういうシェル芸は、人間が一瞬で安全性を読むにはつらいです。何度もpromptを見ていると、確認はだんだん雑になります。毎回同じルールでLLMに読ませる価値はここにあります。

`/path/to/reponame-worktree1`のようなworktreeで作業しているのに、`/path/to/reponame`のような元のcheckoutを検索し始めることもあります。こうなると、無限に近い勢いでPermission許可を求められます。

それでも、`gh`、ローカルDB、ネットワーク、いずれも雑に通すのは怖い。安全側に倒しつつ、プロジェクトルールに沿った操作は止めずに通したい。それが出発点でした。

## だいたい同じ理由で許可/拒否していた

何度も許可/不許可をしていると、「この操作は許可してよい」「これは止めたい」を、だいたい同じ基準で判断していることに気づきます。

通したいもの、止めたいもの、人間に戻したいものを書き出すと、こんな感じでした。

- 通したい: リポジトリ内のread-only調査、プロジェクト定義のtest/lint/build、feature branch上の普通のgit操作
- 止めたい: `python` / `python3`の直接実行、task runnerに定義されていない独自スクリプト、`npx`や`pnpx`での一発実行、リポジトリ外への編集、force push、本番っぽい環境への変更
- 人間に戻したい: ローカルDBのmigrate-downやdropのように、明確に危ないが自分が指示した可能性もあるもの

特につらかったのは、こういうパターンです。
...
- `make test`や`pnpm run lint`を使えばいいのに、Agentが独自の`npx`や`pnpx`を始めようとする
- systemの`python` / `python3`を直接実行する。自分の環境では、最低限`uv run`や`uvx`を使ってほしい
- 「これは毎回ダメ」とコメントして拒否しているのに、似たような操作が何度も出る

`npx`や`pnpx`を避けたいのは、好みの問題だけではありません。npmエコシステムでは、2025年9月の[Shai-Hulud](https://www.csa.gov.sg/alerts-and-advisories/alerts/al-2025-093/)のような広範なサプライチェーン攻撃や、2026年3月の[axios npmパッケージ侵害](https://snyk.io/blog/axios-npm-package-compromised-supply-chain-attack-delivers-cross-platform/)が報告されています。Agentが思いつきで未固定のnpm packageを実行するより、リポジトリに定義されたscriptやlockfileの範囲に寄せたいです。

`gh`を全部denyすると不便ですが、全部allowすると危険です。`Bash(git *)`も同じで、読み取りやfeature branchへのpushは通したいが、force pushやmainへのpushは止めたい。コマンド文字列だけでなく、文脈込みで判断したい領域です。

問題は、この判断を人間が毎回やることです。1日に何十回、多い日は100回以上もPermission promptを見ると、集中して読み続けるのは難しい。結局、反射でYesを押したり、同じような理由でNoを押して、また同じ説明をAIに投げたりしがちです。「こういうコマンドはやめて」とMarkdownに書いても、Agentは長い作業の途中で忘れます。

であれば、その基準をルールに落とし込んで、本来の作業をしているAgentとは別の第三者AIに任せればよい、というのが出発点でした。`fallthrough`で人間に戻す余地も残せれば、ユーザーが明示的に危険操作を指示したときも勝手に通さずに済みます。
...
なお、Codex CLIにはClaude CodeのAuto Modeと同じ名前のmodeはありません。Codex CLI側のDefault、Full Access、`auto_review`は次節で分けて整理します。
...
Codex CLI側の自動化系の仕組みを整理します。2026年4月28日時点のopenai/codexを見ると、少なくとも次のものは別物です。
...
`auto_review`は新しい仕組みで、社内で実際に試した同僚からは「Full accessとの違いが体感わかりづらい(リスクは出るがそのまま進んでいく)」という声も出ていました。Codex本体側のguardian機構として動くので、外から挙動を追うのは少し難しい部分があります。

:::details `auto_review`の中身

`auto_review`については、OpenAI Codexの[app-server README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md)で`approvalsReviewer: "auto_review"`として説明されています。実装を見ると、guardian subagentがコンパクトな会話履歴と対象アクションを読み、risk-basedに判断します。`config.toml`には`auto_review.policy`もあり、固定のguardian prompt全体を置き換えるというより、Policy Configuration部分に渡すポリシーを設定できる形です。

つまり、`auto_review`は「Claude CodeのAuto Modeそのもの」ではありません。Claude CodeのAuto Modeはpermission modeの一つですが、Codex CLIの`auto_review`はapproval requestのreviewerです。`auto_review.policy`でポリシーは書けるものの、Codex本体側のguardian機構として動きます。ccgateはその外側で、`PermissionRequest` hookに流れてきたapproval判断を扱います。
...
この93.9%は、ccgate開発中の検証や設定変更が混ざった全期間の集計です。普段の運用では基本的に97%から100%近くまで自動化されています。多い日では1日236件、別の日にも224件や201件の判定がありました。これを全部自分で押していたら、かなりの集中力が削られていたと思います。
...
自分の環境では、2026年4月1日から4月28日までのClaude Code側判定で、開発中の検証や設定変更を含む全期間集計では93.9%、普段の運用では基本的に97%から100%近くが自動化されています。denyやfallthroughもちゃんと機能しています。体感としても、Claude Codeに長めの調査や実装を任せたときの「戻ってきたら止まっていた」がかなり減りました。Codex CLIでも、同じ考え方を使えるようになっています。
