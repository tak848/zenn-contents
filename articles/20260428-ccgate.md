...
それでも自分はccgateの方を使い続けています。`deny_message`を自分のルール側で管理できる柔軟性と、プロンプト・設定・ログ・metricsがすべて手元にある透明性が大きな理由です。第三者の監視員という立ち位置がhookと相性がよく、Claude / Codexで同じjsonnet書式が使えて`libsonnet`で簡単に共通化できる点も便利です。Auto Mode自体はチューニングできますが、設定の見え方や挙動把握に労力がかかるという声も周囲で耳にします。ccgateはルール・プロンプト・ログを自分の手で詰めやすいので、結局そちらに落ち着きました。
