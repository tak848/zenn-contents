...
そこで、Permission許可を別のAIに任せる[ccgate](https://github.com/tak848/ccgate)というCLIを作ったので紹介させてください。

https://github.com/tak848/ccgate

## 何を作ったか（簡単に）

Claude CodeやCodex CLIでPermission promptが出る直前に、ccgateを挟めるようにしました。

本来は人間が「このToolを実行していいですか？」に答えるところで、ccgateが別のLLMに聞きます。

たとえば、こんな感じです。

| Agentがやろうとしたこと | ccgateの判断 |
| --- | --- |
| リポジトリ内を`rg`で調べる | `allow` |
| プロジェクトに定義されたtest / lintを実行する | `allow` |
| `curl | bash`する | `deny` |
| 未固定の`npx`や`pnpx`で一発実行する | `deny` |
| ローカルDBのmigrate-downなど、最後だけ人間が見たい操作 | `fallthrough` |

ここでいう`fallthrough`はccgate内部の呼び方です。
Claude Code / Codex CLIのhookに`behavior: "fallthrough"`を返すわけではありません。
ccgateが判断しないことで、通常のPermission promptに戻します。

`deny`するときは理由も返せます。単に止めるだけではなく、「そのコマンドではなく、プロジェクトに定義されたscriptを使ってください」のように返せるので、Agentも次の試行で動きを直しやすくなります。

使い方だけ見たい人は[使い方](#使い方)へどうぞ。

## PermissionRequest hookに挟む

ccgateは「Permission promptを全部消す」ためのツールではありません。
Agentが安全に、環境を壊さず、プロジェクトルールに従ったコマンドでシュッと解決できるようにするためのツールです。

そのために使うのが、Claude CodeやCodex CLIの`PermissionRequest` hookです。
PermissionRequest hookは、本来なら人間に確認する場面で、先に外部コマンドを呼び出せる仕組みです。

ccgateはこのhookに入ります。元のAI coding toolがユーザーに確認を出す直前にccgateが呼ばれ、その操作を別のLLMに判定させます。

| ccgate内部の判定 | hookへの返し方 | 何が起きるか |
| --- | --- | --- |
| `allow` | `decision.behavior: "allow"`を返す | 元のTool実行を許可する |
| `deny` | `decision.behavior: "deny"`と`message`を返す | 理由付きで拒否する |
| `fallthrough` | `decision`を返さない | 通常のPermission promptに戻す |

Claude Codeの公式ドキュメントでは、`PermissionRequest`の`decision.behavior`は`allow` / `deny`です。Codex hooksの公式ドキュメントでも、`allow` / `deny`を返すか、判断せず通常のapproval flowに戻す、という扱いになっています。
なので、ccgateの`fallthrough`は「hookの第3の戻り値」ではなく、「ccgateが何も決めずに上流へ戻す」という意味です。

細かい差分として、Claude Codeの`PermissionRequest`は`allow`時の`updatedInput` / `updatedPermissions`や、`deny`時の`interrupt`も定義されています。一方、Codex hooksではそれらのフィールドは将来用で、現時点では返さないように書かれています。ccgateは両方で使える共通部分として、基本的に`allow` / `deny` / 判断しない、の形に寄せています。
...
また、Codex CLIにはClaude CodeのAuto Modeと同じ名前のmodeがあるわけではありません。ただ、自分の体感では、sandboxやapproval policyの範囲でかなり自動的に進む設計になっています。ccgate codexは、その自動判断を置き換えるというより、Codex CLIが最後にapproval promptへ戻そうとした操作を、ccgateのルールで判定するためのものです。
...
ここも、Claude Codeと同じように、普段の判断をルールに落とし込めます。ローカルのread-only調査やプロジェクト定義のtestは通したい。`curl | bash`、リポジトリ外の破壊的操作、意図しない外部通信は止めたい。ローカルDBを壊しそうな操作などは人間に戻したい。Codex CLIのデフォルト挙動を否定したいわけではなく、最後に人間へ戻ってきた判断を、より一貫したルールに寄せたいという話です。
