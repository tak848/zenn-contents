---
title: "97%のPermission確認を自動化するCoding Agent用OSS「ccgate」が誕生した"
emoji: "🛂"
...
:::message
- Claude / CodexのPermission確認をLLMに代行させる`ccgate`というCLIを作った
- もとはAuto Mode的な体験を目指して作り、Auto Modeが使えるようになった今もPlan mode中の確認やCodexへの展開、ログ・metricsの可視化のために使い続けている
- 自分の環境では1ヶ月約2000件のPermission確認のうち、基本的に97%近くが自動化された
:::
...
そこでPermission許可を別のAIに任せる[ccgate](https://github.com/tak848/ccgate)というCLIを作りました。

https://github.com/tak848/ccgate

![PermissionRequest hookで自動的に許可/拒否されている様子](/images/20260428-ccgate/terminal.png)
...
ルールは[jsonnet](https://jsonnet.org/)(JSONのスーパーセットの設定言語)で書きます。書き方は後の節で詳しく触れます。
...
最新の手順は[README](https://github.com/tak848/ccgate)を見てください。ここではClaude Code向けの最短の設定方法を載せます。

やることは、インストール、hook登録、API key設定の3つです。

例えば、miseを使っている場合は以下のようにインストールできます。
...
最後に、Anthropic API keyを設定します（執筆時点で対応しているのはAnthropic公式のAPIのみです）。[こちら](https://platform.claude.com/settings/keys)から作成できます。

```bash
export CCGATE_ANTHROPIC_API_KEY=...
```

デフォルトでは、Haiku 4.5になっていますが、Sonnet 4.6などに設定で上書きも出来ます。ただ個人的にはHaikuをフル活用して月$5程度で運用できていて品質問題もほとんど感じていないのです。

設定ファイルなしだと組み込みのデフォルトルールで動きます。若干デフォルトは厳しめになっているので、カスタマイズしたい場合は、デフォルトを吐き出して編集します。metricsやログをみて個人でルールをブラッシュアップするのがおすすめです。
...
とはいえ、面倒だからといって全部を自動許可するのも怖いです。`gh`も使えるし、ローカルDBを壊すことはあるし、ネットワークにも出られます。AIエージェントが何かを勘違いしたときに、ホスト環境で全部通るのはさすがに緊張感があります。ghのread系のcommandはAllow Listに入れたとしても、`gh api`が必要なシーンは多々あり、これをDenyに入れるとPull Requestのレビュー管理でしんどいし、かといってAllowに入れると、あまりにもなんでも出来てしまうので躊躇してしまいます。
...
:::message
**[追記] Plan mode中でもAuto Modeを有効化できます**

公式 [Settings](https://code.claude.com/docs/en/settings) には `useAutoModeDuringPlan` (デフォルト `true`)という設定があり、Plan mode中でもAuto Modeのセマンティクスが使えます。執筆時点で把握できておらず、本文中の「Auto ModeはClaude Codeのmodeの一つなので、Plan mode中には使えません」 は誤りでした。失礼しました。

https://x.com/oikon48/status/2049085361956274386?s=20

https://x.com/oikon48/status/2049089494033248595?s=20

「Plan modeで計画して、そのまま実装まで進める」運用を取る人にとっては、Plan mode中にAuto Modeが効くのは便利だと思います。ccgateは hook なので mode に依らず動くため、同じ場面でも一緒に走らせられます。

それでも自分の運用ではccgateの方を使い続けています。理由を冷静に整理すると、以下のあたりです。

- `deny_message`を自分のルール側で完全に管理できる(AIに返す文言の独立性・柔軟性)
- プロンプト・設定・ログ・metricsがすべて手元にある(透明性)
- Claude Code / Codex CLI で同じjsonnet書式が使えて、共通化したい場合は `libsonnet` で簡単にまとめられる
- Codex CLI側に `auto_review` が出てきても、結局 `PermissionRequest` hook に乗せるのが一番扱いやすかった
- 監視員として第三者の立ち位置がhookと相性が良い

Auto Mode自体はチューニングできる仕組みですが、設定の見え方や挙動把握に労力がかかるという声も周囲で耳にします。ccgateはルール・プロンプト・ログが手元にあるので、そのあたりを自分の手で詰めやすいのが、結局Auto Modeそのものを自分があまり使わなくなった一番の理由かもしれません。

以下の本文(特に「Plan mode中には使えません」「ccgateを残している理由(Plan mode中も判定できる)」あたり)は当時の理解のまま残しています。記事を読み進める際は、この追記を踏まえてご覧ください。
:::

ccgateの着想には、Claude Codeの[Auto Mode](https://claude.com/blog/auto-mode)が大きく関係しています。
...
技術書典20向けの原稿を書いていた2026年3月30日時点では、私のMax環境ではAuto Modeを使えませんでした。使えたタイミングも一瞬あり、そのときに「Permissionプロンプトが消えるだけで、こんなに作業が進むのか」と感動しました。

### 2026年4月時点の利用条件

2026年4月28日時点では状況が変わっています。公式の[Permission modes](https://code.claude.com/docs/en/permission-modes)によると、Auto ModeはClaude Code v2.1.83以降で利用できます。対象プランはMax、Team、Enterprise、APIです。ただしProでは使えません。Team / Enterpriseでは管理者による有効化が必要で、モデル条件もあります。MaxではOpus 4.7のみ、Team / Enterprise / APIではSonnet 4.6、Opus 4.6、Opus 4.7が対象とされています。ProviderはAnthropic APIのみで、Bedrock、Vertex、Foundryでは利用できないとも書かれています。

技術書典の原稿に書いた「当時使えなかった」は当時の事実ですが、現在の利用条件は変わっています。

### Auto Modeの中身

Auto Modeの中身も公式ドキュメントに整理されています。広すぎる`Bash(*)`などのallowルールがdropされ、read-only操作や作業ディレクトリ内のファイル編集はclassifierを通さずに自動許可されます。classifierが3回連続、または合計20回ブロックすると通常のPermissionプロンプトへフォールバックします。本体に組み込まれているからこそできる作りで、外部hookでは再現しづらい部分です。

### ccgateとの違い

一方、ccgateはClaude CodeとCodex CLIの`PermissionRequest` hookに乗る外付けのゲートです。Auto Modeのようにmodeを切り替えるものではなく、いま使っているmodeの上で動きます。Auto ModeはClaude Codeのmodeの一つなので、Plan mode中には使えません。ccgateはmodeではないので、PermissionRequestに流れてきた操作を判定できます。
...
### ccgateを残している理由(Plan mode中も判定できる)

Auto Modeが使える今でもccgateを外していないのは、表に出ている違いを実際に欲しがっているからです。Plan mode中の確認も判定したい、判断に迷ったら人間に戻したい(`fallthrough`)、プロンプト・設定・ログ・metricsを自分で見える形にしたい。Codex CLIにも同じ考え方を載せたい。Auto Modeはちゃんと作られていますが、Claude Code本体側に閉じた仕組みで、外からは限定的にしか触れません。

Claude CodeのPlan modeは「まず調べて計画してね」というモードですが、Permissionプロンプト自体はdefault modeと同じように適用されます。調査中のAgentが`rg`、`find`、`git`を組み合わせたり、`&&`や`$()`を含むコマンドで動き回ったりすると、それだけで確認が増えます。ccgateはPermissionRequest hookなので、Plan mode中に流れてきた操作も判定できます。Plan modeでの正しさはccgate側のプロンプトに依存していて、hard guaranteeではありません。それでも自分の作業では、「調査しておいて」と投げて戻ってきたらかなり進んでいる、という体験に近づきました。

なお、Codex CLIにはClaude CodeのAuto Modeと同じ名前のmodeはありません。Codex CLI側の`approval_policy` / `sandbox_mode` / `auto_review`は次節で分けて整理します。

## Codex CLIでも同じ問題が出てきた

ここまでClaude Codeの話を中心に書いてきましたが、許可判断をどう扱うかはClaude Codeだけの問題ではありません。

### Codex CLIのapproval周り

Codex CLIは、設定次第でかなり自動で進みます。自分はsandboxを使いつつ、network accessは許可する設定で使っています。その状態だとClaude Codeほど大量にapproval promptが出るわけではありません。

ただ、Codex CLIもapproval policyやsandboxの範囲で通せるものは進み、リスクが高い、または追加の承認が必要と判断されたところでapproval promptが出ます。普段は自動で進みつつ、リスクが高そうなところだけ人間に判断が戻ってくる設計です。本来そこは丁寧に見るべき場所ですが、作業の流れの中で雑に許可してしまうこともあります。

ここも、Claude Codeと同じように、普段の判断をルールに落とし込めます。ローカルのread-only調査やプロジェクト定義のtestは通したい。`curl | bash`、リポジトリ外の破壊的操作、意図しない外部通信は止めたい。ローカルDBを壊しそうな操作などは人間に戻したい。Codex CLIのデフォルト挙動を否定したいわけではなく、approval promptに戻ってきた判断をより一貫したルールに寄せたいだけです。

### Codex CLI側の自動化系を整理する

Codex CLI側の自動化系の仕組みを整理します。Codexの挙動は基本的に`approval_policy`と`sandbox_mode`の組み合わせで決まり、それに加えて`auto_review`という別軸の仕組みがあります。2026年4月28日時点のopenai/codexを見ると、少なくとも次のものは別物です。

| 仕組み | 何をするか | ccgateとの関係 |
| --- | --- | --- |
| 通常運用の組み合わせ | `approval_policy`と`sandbox_mode`の組み合わせ。workspace内の編集やコマンドは進め、internet accessやworkspace外の編集などでapprovalを求める | approval promptに戻るところをccgateで扱える |
| Full Access | `approval_policy = never` + `sandbox_mode = danger-full-access`相当で、workspace外の編集やinternet accessも確認なしで進める | prompt自体が出ない方向なので、ccgateとは目的が違う |
| `auto_review` | 対話的なapproval requestを自動レビューに回し、リスクに応じてallow/denyする | ルール・判定・ログがCodex内に閉じる |
| `ccgate codex` | `PermissionRequest` hookに流れてきた操作を、jsonnetルールとLLM判定でallow/deny/fallthroughする | 手元のルール、ログ、metricsを自分で持てる |

:::details `auto_review`の中身

`auto_review`については、公式の[Configuration: advanced](https://developers.openai.com/codex/config-advanced#approval-policies-and-sandbox-modes)で説明されています。`config.toml`で`approvals_reviewer = "auto_review"`を設定すると、対話的なapproval requestが自動レビューに回されます。`[auto_review]`の`policy`に local reviewer policy instructions を書ける形で、固定のreviewer prompt全体を置き換えるというより、ポリシーを追加で渡す形です。なお、managed環境では`guardian_policy_config`がlocalの`auto_review.policy`より優先されます。

つまり、`auto_review`は「Claude CodeのAuto Modeそのもの」ではありません。Claude CodeのAuto Modeはpermission modeの一つですが、Codex CLIの`auto_review`はapproval requestのreviewerです。ルール記述・判定・ログはCodex内に閉じています。

判定能力という意味ではccgateと重なる部分もありますが、ccgateの強みは別の軸にあります。Claude CodeとCodex CLIで同じjsonnet書式が使え、共通化したい場合は`libsonnet`などで簡単にまとめられます。判定の中身も自分で見える。Codex内で何が起きているか分からなくなる、を避けたいという話に近いです。

:::

### Codex hooks(PermissionRequest)が入った
...
この94.0%は、ccgate開発中の検証や設定変更が混ざった全期間の集計です。開発タイミングで暴発させてしまったタイミングでfallthroughが多く発生した時がありましたが、普段の運用ではruleを改善した効果もあり、dailyで見ると基本的に97%近くまで自動化されています。多い日では1日236件、別の日にも224件や201件の判定がありました。これを全部自分で押していたら、かなりの集中力が削られていたと思います。
...
これは、自分のオタク管理しているdotfilesから派生した都合も大きいです。jsonnetはJSONのスーパーセットなので普通のJSONっぽくも書けますし、共通部分を切り出せるのも便利です。一方で、今のところ設定形式がjsonnetのみなのは少し尖っています。JSONを書いて、拡張子をjsonnetにすれば動きはしますが、純粋にJSON対応できていないだけなので、JSON対応のPRも歓迎です。
...
自分の環境では、2026年4月1日から4月28日までのClaude Code側判定で、開発中の検証や設定変更を含む全期間集計では94.0%、普段の運用では基本的に97%から100%近くが自動化されています。denyやfallthroughもちゃんと機能しています。体感としても、Claude Codeに長めの調査や実装を任せたときの「戻ってきたら止まっていた」がかなり減りました。Codex CLIでも、同じ考え方を使えるようになっています。

Permission許可に疲れている人は、各ツールのAuto Modeと併せてぜひ試してみてください。`--dangerously-skip-permissions`にしていた人も、これを機にやめられるのではないでしょうか！
