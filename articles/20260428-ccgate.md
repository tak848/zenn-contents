...
### [追記] Plan mode中でもAuto Modeを有効化できた

公式の[Settings(Available settings)](https://code.claude.com/docs/en/settings#available-settings)には `useAutoModeDuringPlan` (デフォルト `true`)があり、Plan mode中でもAuto Modeのセマンティクスが使えます。執筆時点で把握できておらず、本文の「Plan mode中には使えません」 は誤りでした。失礼しました。Oikonさん、ありがとうございます！Release noteは常に眺めているものの、追い切れていませんでした。

https://x.com/oikon48/status/2049085361956274386?s=20

「Plan modeで計画してそのまま実装まで進める」運用ならAuto Modeが効くと便利です。ccgateはhookでmodeに依らず動くため、同じ場面でも一緒に走らせられます。

それでも自分はccgateの方を使い続けています。`deny_message`を自分のルール側で管理できる柔軟性と、プロンプト・設定・ログ・metricsがすべて手元にある透明性が大きな理由です。第三者の監視員という立ち位置がhookと相性がよく、Claude / Codexで同じjsonnet書式が使えて`libsonnet`で簡単に共通化できる点も便利です。Auto Mode自体はチューニングできますが、設定の見え方や挙動把握に労力がかかるという声も周囲で耳にします。

突き詰めると、自分がそれまでやっていた「コマンドを見る → 確認 → 判断」 というループを、ルール・プロンプト・ログが手元に見える透明なhookとして外に出せたことが大きいです。各ツールの機構から独立していることで、振り回されずに安全に動くことが魅力だなと感じています。

以下の本文は当時の理解のまま残し、該当箇所には取り消し線で訂正を入れています。
...
一方、ccgateはClaude CodeとCodex CLIの`PermissionRequest` hookに乗る外付けのゲートです。Auto Modeのようにmodeを切り替えるものではなく、いま使っているmodeの上で動きます。Auto ModeはClaude Codeのmodeの一つなので、~~Plan mode中には使えません~~(追記参照)。ccgateはmodeではないので、PermissionRequestに流れてきた操作を判定できます。
...
| Plan mode中 | ~~Auto Modeとは別modeなので、Plan中には使えない~~(追記参照) | Plan mode中のPermissionRequestも判定できる |
...
自分の環境で`ccgate claude metrics`を見たところ、2026年4月1日から4月28日までに、Claude Code側だけで2,079件のPermissionRequest相当の判定がありました。
