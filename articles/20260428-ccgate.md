...
| `auto_review` | 対話的なapproval requestを自動レビューに回し、リスクに応じてallow/denyする | Codex本体内で動くreviewer。ccgateは外付けhookで同等の判定を行う |
...
つまり、`auto_review`は「Claude CodeのAuto Modeそのもの」ではありません。Claude CodeのAuto Modeはpermission modeの一つですが、Codex CLIの`auto_review`はapproval requestのreviewerです。`auto_review.policy`でポリシーは書けるものの、Codex本体内のreviewerとして動きます。ccgateはその外側で、`PermissionRequest` hookに流れてきたapproval判断を扱います。
