...
「Plan modeで計画してそのまま実装まで進める」運用ならAuto Modeが効くと便利です。ccgateはhookでmodeに依らず動くため、同じ場面でも一緒に走らせられます。なぜそれでもccgateを使い続けているかは、後ろの「ccgateを残している理由」 を厚くしてあります。
...
Auto Modeが使える今でもccgateを外していないのは、表に出ている違いを実際に欲しがっているからです。大きいのは、`deny_message`を自分のルール側で管理できる柔軟性と、プロンプト・設定・ログ・metricsがすべて手元にある透明性です。第三者の監視員という立ち位置もhookと相性がよく、Claude / Codex CLIで同じjsonnet書式が使えて、`libsonnet`で簡単に共通化できる点も便利です。Auto Mode自体はチューニングできる仕組みですが、設定の見え方や挙動把握に労力がかかるという声も周囲で耳にします。Auto Modeはちゃんと作られていますが、Claude Code本体側に閉じた仕組みで、外からは限定的にしか触れません。

突き詰めると、自分がそれまでやっていた「コマンドを見る → 確認 → 判断」 というループを、ルール・プロンプト・ログが手元に見える透明なhookとして外に出せたことが大きいです。各ツールの機構から独立していることで、振り回されずに安全に動くことが魅力だと感じています。
