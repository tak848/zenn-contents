...
Permission promptはもちろん大事な安全装置です。
ただ、本来ユーザーに確認を出す直前に外部コマンドを呼べる`PermissionRequest` hookがあります。
そのhookに別のAIを挟んで、許可する / 拒否する / 人間に戻す、を判断させる[ccgate](https://github.com/tak848/ccgate)というCLIを作ったので紹介させてください。

https://github.com/tak848/ccgate

## 何を作ったか

ccgateは、Claude CodeやCodex CLIの`PermissionRequest` hookに入るCLIです。

本来は人間が「このToolを実行していいですか？」に答えるところで、ccgateが別のLLMに聞きます。
そこで`allow`、`deny`、または`fallthrough`を返します。
...
ここでいう`fallthrough`はccgate内部の呼び方で、hookに第3の戻り値を返すわけではありません。ccgateが判断しないことで、通常のPermission promptに戻します。
...
ccgateはこのhookで呼ばれ、その操作を別のLLMに判定させます。
