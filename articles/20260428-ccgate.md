...
公式の[Settings(Available settings)](https://code.claude.com/docs/en/settings#available-settings)には `useAutoModeDuringPlan` (デフォルト `true`)があり、Plan mode中でもAuto Modeのセマンティクスが使えます。執筆時点で把握できておらず、本文の「Plan mode中には使えません」 は誤りでした。失礼しました。
...
突き詰めると、自分がそれまでやっていた「コマンドを見る → 確認 → 判断」 というループを、ルール・プロンプト・ログが手元に見える透明なhookとしてccgateが代行してくれる──そこが一番ハマっている気がします。
