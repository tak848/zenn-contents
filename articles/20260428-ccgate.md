...
この記事は、そのPermission許可を別のAIに任せる[ccgate](https://github.com/tak848/ccgate)というツールを作った話です。Agentには自由に動いてほしい。でも、環境を壊されず、プロジェクトルールに従ったコマンドでシュッと解決してほしい。その間を埋めるための小さなゲートです。

## 原点: Permission promptで作業が止まる

Permission promptはもちろん大事な安全装置です。Claude Codeにファイル編集やコマンド実行を任せていると、かなりの頻度で「このToolを実行していいですか？」と聞かれます。

原点はシンプルで、安全に、環境を壊されずにAgentを動かしたい、ということでした。Permission promptはそのための仕組みですが、許可/拒否ばかりしているとめんどい。特に、長めのタスクを任せて少し放置し、戻ってきたら全然進まずに止まっているとかなりげんなりします。
...
また、Codex CLIにはClaude CodeのAuto Modeと同じ名前のmodeがあるわけではありません。ただ、自分の体感では、sandboxやapproval policyの範囲でかなり自動的に進む設計になっています。ccgate codexは、その自動判断を置き換えるというより、Codex CLIが最後にapproval promptへ戻そうとした操作を同じルールで判定するためのものです。
...
Auto Modeにもdenialの記録やretryの導線はあります。ここで言いたいのは、ccgateではjsonnetの`deny_message`として、Claude Code / Codex CLIに返す文言を自分で置ける、という話です。
