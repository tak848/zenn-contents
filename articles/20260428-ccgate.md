...
| `auto_review` | 対話的なapproval requestを自動レビューに回し、リスクに応じてallow/denyする | ルールは`[auto_review].policy`、判定はCodex内のreviewer。ccgateはjsonnetで書き、別建てのLLM(デフォルトHaiku)が判定する |
...
つまり、`auto_review`は「Claude CodeのAuto Modeそのもの」ではありません。Claude CodeのAuto Modeはpermission modeの一つですが、Codex CLIの`auto_review`はapproval requestのreviewerです。`auto_review.policy`でポリシーは書けるものの、ルール記述はCodexの`[auto_review].policy`に閉じ、判定はCodex本体内のreviewerが行います。ccgateはその外側で、`PermissionRequest` hookに流れてきたapproval判断を扱います。ルールはjsonnetで自然言語の`allow` / `deny` / `environment`を書き、別建てのLLM(デフォルトHaiku)が判定するので、ルール記述・判定主体・ログ/metricsの置き場が`auto_review`とは別になります。
