...
それでも自分はccgateの方を使い続けています。`deny_message`をルール側で完全に管理できる・プロンプト・設定・ログ・metricsが手元にある(透明性)・Claude / Codexで同じjsonnet書式が使えて`libsonnet`で簡単に共通化できる・第三者の監視員という立ち位置がhookと相性が良い、あたりが理由です。Auto Mode自体はチューニングできますが、設定の見え方や挙動把握に労力がかかるという声も周囲で耳にします。ccgateはルール・プロンプト・ログを自分の手で詰めやすいので、結局そちらに落ち着きました。
