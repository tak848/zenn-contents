...
Claude Codeにファイル編集やコマンド実行を任せていると、かなりの頻度で「このToolを実行していいですか？」と聞かれます。もちろん大事な安全装置です。ですが、長めのタスクを任せて少し離席し、戻ってきたら最初の`rg`で止まっていると、ちょっと泣けます。
...
Codex CLIでも、sandboxやapproval policyを使いながら作業していると、同じように「これは通してよいが、これは止めたい」という境界が出てきます。
...
最後に、Auto Mode、Claude Code hooks、Codex hooksの仕様はかなり速く変わっています。この記事の仕様に関する事実は、2026年4月27日時点で公式ドキュメントを確認した内容に基づいています。
